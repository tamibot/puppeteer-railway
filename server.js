...
// ---------- Lanzador (stealth + userData + proxy opcional) ----------
async function launchBrowser(){
  const args = [
    "--no-sandbox","--disable-setuid-sandbox","--disable-dev-shm-usage",
    "--no-zygote","--single-process","--window-size=1366,768"
  ];
  ...
  return puppeteerExtra.launch({
    ...
    headless: true,
    args,
    userDataDir: "/usr/src/app/.chromedata"
  });
}
...
// ---------- Intento local robusto (stealth + fallbacks) ----------
  ...
  for (const waitUntil of ["networkidle2","domcontentloaded"]){
    for (let intento=1; intento<=2; intento++){
      try{
        ...
        // esperar si hay challenge tipo “verificando su navegador…”
        await Promise.race([
          page.waitForFunction(
            () => !/just a moment|please wait|checking your browser|ser humano/i.test(document.body.innerText),
            { timeout: 15000 }
          ),
        ...
        await browser.close();
        return html;
      }catch(e){ lastErr = e; await new Promise(r=>setTimeout(r, 1000*intento)); }
    }
  }
...
// ---------- Fallback con proveedor (ejemplo: ScraperAPI) ----------
...
// ---------- GET /html: HTML completo con fallback ----------
...
// ---------- GET /screenshot: para pruebas manuales ----------
